...
using namespace MA5;
//CARO: Directory
TString rootdi="rootfiles"; // previous : "rootfiles_ok1";
...
//CARO: Samples
 ...
 samplelist.push_back( "t-1j-Kappa-gct");         
 samplelist.push_back( "t-1j-Kappa-gut");         
 samplelist.push_back( "SingleTop_W_madspin"); 
 samplelist.push_back( "SingleTop_s_madspin"); 
 samplelist.push_back( "SingleTop_t_4FS_madspin"); 
 ...
 samplelist.push_back("ZToLL50-3Jets_sm-no_masses_split");  
...
// Get Histograms 
 ...
 for(unsigned int i= 0;  i<samplelist.size(); i++){
  ...
  // CARO : CHECK THE LINK
  // mvagut
     TFile * tmp1 = new TFile( (rootdi+"/mva_gut/outputmva_"+samplelist[i]+".root").Data() );
     tmp1->cd();
     TString name1 = "MVA_"+samplelist[i];
     ...
     if ((mylist.GetName(samplelist[i].Data())).find("Signal") == std::string::npos) {
        ...
        if ((mylist.GetName(samplelist[i].Data())).find("gut") !=std::string::npos) {
           // gut :
  ...
  // CARO : CHECK THE LINK
  // mvagct
     TFile * tmp2 = new TFile( (rootdi+"/mva_gct/outputmva_"+samplelist[i]+".root").Data() );
     tmp2->cd();
     TString name2 = "MVA_"+samplelist[i];
 ...
 float BR_limit_gut = 0.0031/100;   
 float BR_limit_gct = 0.016/100;

   cout << " BR_gut_limit " << BR_limit_gut << endl;
   cout << " BR_gct_limit " << BR_limit_gct << endl;
...
// gct
 ...
 for (int k1=0; k1<nloop1-1; k1++) {
    ...
    for (int k2=0; k2<nloop2-1; k2++) {
       ...
       scaling2= br_loop2/(BR_limit_gct);
 ...
 h2_best_sig->GetYaxis()->SetTitle("BR_{gct}");
 h2_best_sig->GetXaxis()->SetTitle("BR_{gut}");
